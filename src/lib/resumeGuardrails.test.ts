...
import { looksLikeInjection, validateResumeOutput, parseLLMJson, looksLikeJobRequest, detectInputLocale, sanitizeResumeHard, looksLikePromptEcho } from "./resumeGuardrails";
...
describe("looksLikePromptEcho (anti-placeholder)", () => {
  it("detecta eco do template do prompt", () => {
    expect(looksLikePromptEcho('"role": "cargo ajustado para a vaga"')).toBe(true);
    expect(looksLikePromptEcho('{"skills": ["skill1", "skill2"]}')).toBe(true);
    expect(looksLikePromptEcho("bullet1 e bullet2")).toBe(true);
  });

  it("aceita conteúdo real gerado para a vaga", () => {
    const real =
      '{"role":"Desenvolvedor Python Senior","summary":"Profissional com 8 anos em automação e dados, alinhado à vaga de Python na Acme.","skills":["Python","FastAPI","Docker"]}';
    expect(looksLikePromptEcho(real)).toBe(false);
  });

  it("não dispara com texto vazio", () => {
    expect(looksLikePromptEcho("")).toBe(false);
  });
});
