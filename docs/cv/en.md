# Samuel Andrade Fonseca de Medeiros

> Data Analyst & Full Stack Developer

Brasília-DF · (61) 99119-1722 · samuelandrademedeiros@gmail.com  
LinkedIn: https://www.linkedin.com/in/samuelandrademedeiros

## Objective

Data and Product Analyst | Creation of Dashboards and Strategic Insights | BI and Performance

## Professional Summary

Information Technology professional with experience in data analysis and full stack development. Experience in creating Power BI dashboards, SQL queries, and automation of routines with Python, as well as development of complete web platforms with React, Next.js, and FastAPI. Experience with API and payment integrations, relational databases, Docker and Cloudflare infrastructure, and applications with local LLMs and RAG. Knowledge of Data Science and Machine Learning applied to data analysis and interpretation.

## Work Experience

### Full Stack Developer & Data Analyst — Freelancer (2025 – Present)

- Development and maintenance of complete web platforms (frontend and backend) with React, Next.js, TypeScript, and FastAPI
- Creation of interactive dashboards and data pipelines with Power BI, SQL, and Python
- Payment integration with Stripe Connect and secure authentication (JWT, 2FA)
- Building of data extraction systems (scraping) with semantic RAG search and local LLMs
- Infrastructure and deployment with Docker, Cloudflare (Tunnel, Pages, R2, D1), and CI/CD (GitHub Actions)
- Automation of E2E and unit tests with Playwright, Vitest, and pytest — more than 2,000 automated tests
- Five web projects in production, including marketplace, data platform, and bilingual blog

### National Water Agency (ANA) — Data Analyst (2025)

- Analysis and handling of water systems data to generate insights
- Creation of Power BI dashboards with strategic indicators
- Automation of data collection and processing routines with Python and SQL
- Manipulation and optimized queries in databases
- Technical support to internal areas

### Global Hitss — Level 1 Support Technician (2024 – 2025)

- Diagnosis and resolution of problems in hardware, software, and networks
- Monitoring of systems and IT processes
- Support in formatting, backup, and maintenance of systems
- Technical support to users via phone, email, and chat
- Use of Microsoft Azure and Microsoft 365

### Freelancer — Technical Assistant (2021 – 2023)

- Installation, configuration, and maintenance of computers, printers, and peripherals
- On-site and remote technical support to users (hardware and software)
- Implementation of systems and new technologies
- User and permission management (Office 365, Active Directory)

## Education

Postgraduate in Data Science and Big Data Analytics — IESB University Center (in progress)

Bachelor's Degree in Systems Analysis and Development — IESB University Center

## Technical Skills

- **Programming and Automation:** Python (FastAPI, automation, ETL, scraping), JavaScript/TypeScript (React, Next.js, Node.js), API integration, process automation
- **Databases and BI:** PostgreSQL, SQL Server, SQLite, Power BI, Power Query, DAX, ETL, data transformation and integration (JSON, XML)
- **Data & Machine Learning:** TensorFlow, R, PyTorch, Hadoop, Spark, data modeling and analysis, local LLMs (Ollama), semantic RAG, MCP
- **DevOps and Infrastructure:** Docker, Cloudflare (Tunnel, Pages, R2, D1), Linux, CI/CD (GitHub Actions), Git, GitHub, GitLab
- **Testing and Quality:** Playwright, Vitest, pytest, E2E and unit tests
